...
class UISet {
	...
	void addNumSet( int l, int c, String label, String pos, integer i, int s, color C ) {
		addToIndices( l, c );
		float x = X + margin + (l * column_width) + ( ( (ceil(Hx) - Hx) * column_width ) / 2f );
		float y = Y + margin + (c * line_height) + ( ( (ceil(Vx) - Vx ) * line_height) / 2f );
		set = (UIElement[]) append( set, new NumSetButton( x, y, Hx * column_width, Vx * line_height, label, pos, i, s, C) );
	}
	void addNumSet(String label, String pos, integer inc, int s) {
	 ...
	 void addNumSet(String label, String pos, integer inc, int s, color c ) {
		if ( columning ) {
			this.addNumSet( I, J + C, label, pos, inc, s, c );
			C += ceil( Vx );
		} else if ( rowing ) {
			this.addNumSet( I + C, J, label, pos, inc, s, c );
			C += ceil( Hx );
		} else println( "addNumSet failed: not columning nor rowing." );
	}
	//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	void addShapeListSelect(int l, int c, String label, String pos, PShape par, integer nu) {
		addToIndices( l, c );
		float x = X + margin + (l * column_width) + ( ( (ceil(Hx) - Hx) * column_width ) / 2f );
		float y = Y + margin + (c * line_height) + ( ( (ceil(Vx) - Vx ) * line_height) / 2f );
		set = (UIElement[]) append( set, new ShapeListSelect( x, y, Hx * column_width, Vx * line_height, label, pos, par, nu ) );
	}
	//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	void addFilterBar(String label, String pos, String[] l, integer n, bool e) {
		...
		else println( "addFilterBar failed: not columning nor rowing." );
	}
	//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
class NumSetButton extends UIElement {
	...
	NumSetButton(float x, float y, float w, float h, String str, String pos, integer i, int s, color c) {
		super(x, y, w, h);
		incumbency = i;
		set = s;
		this.setLabel( str, pos );
...
class PlusMinus_Int extends UIElement {
	...
	PlusMinus_Int(float x, float y, float w, float h, String str, String pos, integer i, int step) {
		...
		incumbency = i;
		float bw = 0.25 * w;
		plus = new NumAddButton_Int(x + (w-bw), y, bw, h, i, step);
		minus = new NumAddButton_Int(x, y, bw, h, i, -step);
...
class TextDisplay_vertical extends UIElement {
	...
	void mouseWheel( boolean Q, MouseEvent E ){
		if( Q ){
			if( d != null ){
				scroll.n = constrain( scroll.n - (12*E.getCount()), scrollbar.min, scrollbar.max );
				scrollbar.update();
				this.render();
			}
		}
...
class TextDisplay_horizontal extends UIElement {
	...
	void mouseWheel( boolean Q, MouseEvent E ){
		if( Q ){
			if( d != null ){
				scroll.n = constrain( scroll.n - (8*E.getCount()), scroll_min, left.w );
				this.render();
			}
		}
	}
...
class ShapeListSelect extends UIElement{
	...
	ShapeListSelect( float x, float y, float w, float h, String str, String pos, PShape parent, integer n ){
		super(x, y, w, h);
		this.setLabel( str, pos );
		this.parent = parent;
		selected = n;
		...
		vex = new PVector[N];
		for( int i = 0; i < N; ++i ){
			BB bb = calc_PShape_BB( parent.getChild(i) );
			//parent.getChild(i).translate(-bb.x, -bb.y);
			vex[i] = new PVector( -bb.x, -bb.y, (0.85 * unit_h) / bb.h );
			if( bb.w * vex[i].z > w-16 ){
				vex[i].z = (w-16) / bb.w;
			}
			vex[i].x *= vex[i].z;
			vex[i].y *= vex[i].z;
			vex[i].x += 0.5 * ((w-16) - (vex[i].z * bb.w));
			vex[i].y += (unit_h * i) + ( 0.5 * (unit_h - (vex[i].z * bb.h)) );
			//println( i, vex[i].x, vex[i].y, vex[i].z );
		}

		scroll = new floating_point( 0 );
		scrollbar = new Vertical_Scrollbar( x + w - 16, y, 16, h, scroll, unit_h+m, 0, 0 );
		scrollbar.update( unit_h * N );
	}
	int Y(){
		int Y = floor(( mouseY - scroll.n - y ) / (unit_h) );//, 0, cb.settlement.directive_queue.length -1 );
		if( Y < 0 || Y >= parent.getChildCount() ) Y = -1;
		return Y;
	}
	void mouseMoved( boolean Q ){
		int pmou = moused_over_unit;
	...
	void mouseWheel( boolean Q, MouseEvent E ){
		if( Q ){
			scroll.n = constrain( scroll.n - (20*E.getCount()), scrollbar.min, scrollbar.max );
			scrollbar.update();
		}
	}
	...
	void display(ColorScheme CS){
		...
		stroke( CS.dim ); strokeWeight(2);
		fill( 40 );
		rect( x+1, y+1, w-18, h-2, 3, 3, 3, 3 );

		pushMatrix();
		translate( x + m, y + scroll.n + m );
		...
		if( moused_over_unit >= 0 ){
			noStroke();
			fill( CS.bright );
			rect( 1, unit_h * moused_over_unit, w-16, unit_h );
		}
		if( selected.n >= 0 ){
			noStroke();
			fill( CS.brighter );
			rect( 1, unit_h * selected.n, w-16, unit_h );
		}
		
		for( int i = 0; i < parent.getChildCount(); ++i ){
			pushMatrix();
			translate( vex[i].x, vex[i].y );
			scale( vex[i].z );
			shape( parent.getChild(i), 0, 0 );
			//fill(255,0,0); circle( 0, 0, 1 );
			popMatrix();
		}
		popMatrix();
		scrollbar.display(CS);
